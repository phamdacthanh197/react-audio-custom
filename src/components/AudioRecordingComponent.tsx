import React, { useState, useEffect, ReactElement, Suspense, useMemo } from "react";
...
const AudioRecorder: (props: Props) => ReactElement = ({
  ...
  const genFileWithoutDownload = async (blob: Blob): Promise<File> => {
    if (downloadFileExtension !== "webm") {
      console.warn(
    ...
    const downloadBlobV2 = true
      ? await convertToDownloadFileExtension(blob)
      : blob;
    const fileExt = true ? downloadFileExtension : "webm";
    const fileName = `audio_recording.${fileExt}`;
    const file = new File([downloadBlobV2], fileName, { type: `audio/${fileExt}` });
    return file;
  };
